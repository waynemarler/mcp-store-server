...
async function testMCPStoreServer() {
  ...
  console.log('✅ MCP Protocol Response:', mcpResult.result ? 'Success' : 'Error');
  ...
  console.log('✅ Health check:', healthResult.healthy ? 'Healthy' : 'Unhealthy');

  // Test 6: Clean up - delete test server
  console.log('\n6️⃣ Cleaning up test server...');
  const deleteResponse = await fetch(`${BASE_URL}/api/registry?id=${serverId}`, {
    method: 'DELETE'
  });
